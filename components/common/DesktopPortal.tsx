...
function DesktopPortal({ title, items, initial, subtitle, onLogout, children }: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const { t } = useTranslation();

  const displaySubtitle = subtitle ?? t('malvanMunicipal');
      ...
      <View style={styles.sidebar}>
        <View style={styles.brand}>
          ...
          <View style={styles.brandCopy}>
            ...
            <Text style={styles.brandSub} numberOfLines={1}>{displaySubtitle}</Text>
          </View>
        </View>

        <Text style={styles.menuLabel}>{t('portalMenu').toUpperCase()}</Text>
      ...
      <View style={styles.main}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.pageSub}>{t('municipalServicePortal')}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  navItem: { minHeight: 46, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 12 }, navItemActive: { backgroundColor: COLORS.primary, ...SHADOWS.button }, navItemHover: { backgroundColor: 'rgba(11, 79, 138, 0.05)' }, navLabel: { color: COLORS.textMuted, fontSize: 14, fontWeight: '700' }, navLabelActive: { color: COLORS.white, fontWeight: '800' }, pressed: { opacity: 0.82 },
  logout: { marginTop: 'auto', minHeight: 46, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 12, backgroundColor: '#FEF2F2' }, logoutHover: { backgroundColor: '#FEE2E2' }, logoutText: { color: COLORS.danger, fontSize: 14, fontWeight: '800' },
